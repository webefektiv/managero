...
jQuery(document).ready(function () {
    ...
    if (jQuery('.overall-site-aplicnts').length > 0) {
        ...
        jQuery('.overall-site-resaplicnts').html(loding_strng_mkup);
...
jQuery(document).on('click', '.jobsearch-delete-fav-job', function () {
    ...
    request.done(function (response) {
        ...
        if (typeof response.msg !== 'undefined' && response.msg != '') {
            ...
            jQuery('.jobsearch-table-row' + '[data-job='+ this_id + ']').remove();

            _this.parents('tr').fadeOut();
            this_loader.attr('class', this_loader_b_icon);
        }

        if(response.stare == 'removed'){

            var jobCurent = jQuery('.jobslist-job' + '[data-job=' + this_id + ']');
            jobCurent.remove();
            var test = jQuery('.listaRezervate').find('div.col-12').length;

            if(test == 0){
                console.log('lista goala');
                jQuery('.listaRezervate').append('<h2 class="noJobs">Momentan nu ai selectat joburi in aceasta sectiune.</h2>');
            } else{
                console.log('mai sunt');
            }
        }
...
jQuery(document).on('click', '.jobsearch-delete-rej-job', function () {
    var _this = jQuery(this);
    var this_id = _this.attr('data-id');
    var this_loader = jQuery(this).find('i');
    ...
    var request = jQuery.ajax({
        ...
        data: {
            job_id: this_id,
            action: 'jobsearch_remove_user_rej_job_from_list',
        },
        dataType: "json"
    });
    ...
    request.done(function (response) {
        ...
        if(response.stare == 'removed'){
            ...
            var jobCurent = jQuery('.jobslist-job' + '[data-job=' + this_id + ']');
            jobCurent.remove();
            var test = jQuery('.listaRespinse').find('div.col-12').length;

          if(test == 0){
              console.log('lista goala');
              jQuery('.listaRespinse').append('<h2 class="noJobs">Momentan nu ai selectat joburi in aceasta sectiune.</h2>');
          } else{
              console.log('mai sunt');
          }
        }
...
jQuery(document).on('click', '.jobsearch-delete-apd-job', function () {
    var _this = jQuery(this);
    var this_id = _this.attr('data-id');
    var this_loader = jQuery(this).find('i');
    ...
    var request = jQuery.ajax({
        ...
        data: {
            job_id: this_id,
            action: 'jobsearch_remove_user_apd_job_from_list',
        },
        dataType: "json"
    });
    ...
    request.done(function (response) {
        ...
        if(response.stare == 'removed'){
            ...
            var jobCurent = jQuery('.jobslist-job' + '[data-job=' + this_id + ']');
            jobCurent.remove();
            var test = jQuery('.listaDirect').find('div.col-12').length;

            if(test == 0){
                console.log('lista goala');
                jQuery('.listaDirect').append('<h2 class="noJobs">Momentan nu ai selectat joburi in aceasta sectiune.</h2>');
            } else{
                console.log('mai sunt');
            }
        }
...
jQuery(document).on('click', '.jobsearch-delete-applied-job-2', function () {
    var _this = jQuery(this);
    var this_id = _this.attr('data-id');
    var this_key = _this.attr('data-key');
    var this_loader = jQuery(this).find('i');
    ...
    var request = jQuery.ajax({
        ...
        data: {
            job_id: this_id,
            job_key: this_key,
            action: 'jobsearch_remove_user_applied_job_from_list_2',
        },
        dataType: "json"
    });
...
jQuery(document).on('click', '.reserved-cand-to-intrview', function (e) {
    e.preventDefault();
    var _this = jQuery(this);
    var _job_id = _this.attr('data-jid');
    var _candidate_id = _this.attr('data-cid');
    ...
    if (_this.hasClass('ajax-enable')) {
        ...
        var request = jQuery.ajax({
            ...
            data: {
                _job_id: _job_id,
                _candidate_id: _candidate_id,
                action: 'jobsearch_applicant_to_reserved_by_employer',
            },
            dataType: "json"
        });
...
jQuery(document).on('click', '.blacklist-candidate', function (e) {
    e.preventDefault();
    var _this = jQuery(this);
    var _job_id = _this.attr('data-jid');
    var _candidate_id = _this.attr('data-cid');
    ...
    if (_this.hasClass('ajax-enable')) {
        ...
        var request = jQuery.ajax({
            ...
            data: {
                ...
                _candidate_id: _candidate_id,
                _employer_id: _employer_id,
                action: 'jobsearch_blacklist_candidate',
            },
            dataType: "json"
        });
...
function jobsearch_alljobs_apps_count_load() {
    ...
    request.done(function (response) {
        if ('undefined' !== typeof response.appcounts) {
            ...
            jQuery('.overall-site-resaplicnts').html(response.resappcounts);
    ...
    request.fail(function (jqXHR, textStatus) {
        ...
        jQuery('.overall-site-resaplicnts').html('0');
        jQuery('.overall-site-rejaplicnts').html('0');
    });
    return false;
}
//
jQuery('.-collapsed-target').click(function () {
     var  topDiv = jQuery(this).prev();
    jQuery('.acf-icon',topDiv).click(); 
});
