...
if (!function_exists('jobsearch_time_elapsed_string')) {
    ...
    function jobsearch_time_elapsed_string($ptime, $before = '', $after = '') {
        if ($ptime != '') {
            return $before . human_time_diff($ptime, current_time('timestamp')) . " " . esc_html__('ago', 'wp-jobsearch') . $after;
        } else {
            return '';
        }
    }
...
function jobsearch_dependent_dropdown_list_html($dropdown_field_options, $field_counter, $custom_field_saved_data, $dropdown_field_name_db_val, $parent_id = '0') {
    ...
    if (isset($dropdown_field_options[$parent_id]['label'][$field_counter]) && !empty($dropdown_field_options[$parent_id]['label'][$field_counter]) && is_array($dropdown_field_options[$parent_id]['label'][$field_counter]) && sizeof($dropdown_field_options[$parent_id]['label'][$field_counter]) > 0) {
        ...
        foreach ($dropdown_field_options as $opt_field_key => $opt_field_list) {
            $field_labels = isset($opt_field_list['label'][$field_counter]) ? $opt_field_list['label'][$field_counter] : '';
            $field_vals = isset($opt_field_list['value'][$field_counter]) ? $opt_field_list['value'][$field_counter] : '';
            $field_ids = isset($opt_field_list['id'][$field_counter]) ? $opt_field_list['id'][$field_counter] : '';
            $field_par_ids = isset($opt_field_list['par_id'][$field_counter]) ? $opt_field_list['par_id'][$field_counter] : '';
            ...
            ?>
            <div <?php echo ($opt_field_key > 0 ? 'id="drp-field-' . $opt_field_key . '"' : '') ?> class="jobsearch-profile-select" style="display: <?php echo ($opt_field_key > 0 ? $display_str : 'inline-block') ?>;">
                <select class="selectize-select111 jobsearch-mdep-drpdwn">
                    ...
                    if (!empty($field_labels) && is_array($field_labels)) {
                        ...
                        foreach ($field_labels as $sin_field_label) {
                            ...
                            ?>
                            <option data-id="<?php echo ($sin_field_id) ?>" data-parid="<?php echo ($sin_field_par_id) ?>" <?php echo (!empty($drpdwns_dbval_arr) && in_array($sin_field_val, $drpdwns_dbval_arr) ? 'selected="selected"' : '') ?> value="<?php echo urldecode(sanitize_title($sin_field_val)) ?>"><?php echo ($sin_field_label) ?></option>
                            <?php
                            $sin_field_count++;
                        }
                    }
        ...
        ?>
        <input id="depdrpdwn_fields_val_<?php echo ($field_rand_num) ?>" type="hidden" name="<?php echo ($dropdown_field_name) ?>" value="<?php echo ($dropdown_field_name_db_val) ?>">
        <script>
            ...
            if (!empty($job_deadline_date)) {
                ...
                if ($job_view == 'view-medium') {
                    if ($dead_line_flag) {
                        ...
                        echo '<time datetime="' . date_i18n('Y-m-d H:i:s', $job_deadline_date) . '">' . esc_html__("Deadline ", "wp-jobsearcj") . $jobsearch_last_date_formated . '</time>';
                    }
                    ...
                    if ($dead_line_flag) {
                        ...
                        echo '<li>' . esc_html__("Deadline ", "wp-jobsearcj") . '' . $jobsearch_last_date_formated . '</li>';
                    }
                    ...
                    if ($dead_line_flag) {
                        ...
                        echo ' <small><i class="jobsearch-icon jobsearch-calendar"></i> ' . esc_html__("Deadline ", "wp-jobsearcj") . '' . $jobsearch_last_date_formated . '</small>';
                    }
                } else {
                    if ($job_deadline_date != '' && ($job_deadline_date) <= $current_date) {
                        echo '<li class="deadline-closed"><i class="jobsearch-icon jobsearch-calendar"></i> ' . esc_html__('Application closed.', 'wp-jobsearch') . '</li>';
                    } else {
                        echo '<li><i class="jobsearch-icon jobsearch-calendar"></i> ' . esc_html__("Deadline ", "wp-jobsearcj") . '' . $jobsearch_last_date_formated . '</li>';
                    }
                }
            }
        }
...
function jobsearch_notification_count_in_admin_menu() {
    ...
    wp_reset_postdata();
    $jobs_pending_status = '';
    if (isset($jobs_query->found_posts) && $jobs_query->found_posts > 0) {
        $pending_title = esc_html__('Pending Job(s)', 'wp-jobsearch');
        $jobs_pending_status = '<span class="update-plugins"><span class="plugin-count" title="' . $pending_title . '">' . $jobs_query->found_posts . '</span></span>';
    }
    ...
    $emp_pending_status = '';
    if (isset($emplo_query->found_posts) && $emplo_query->found_posts > 0) {
        $pending_title = esc_html__('Pending Employer(s)', 'wp-jobsearch');
        $emp_pending_status = '<span class="update-plugins"><span class="plugin-count" title="' . $pending_title . '">' . $emplo_query->found_posts . '</span></span>';
    }
    ...
    $cand_pending_status = '';
    if (isset($cand_query->found_posts) && $cand_query->found_posts > 0) {
        $pending_title = esc_html__('Pending Candidate(s)', 'wp-jobsearch');
        $cand_pending_status = '<span class="update-plugins"><span class="plugin-count" title="' . $pending_title . '">' . $cand_query->found_posts . '</span></span>';
    }
