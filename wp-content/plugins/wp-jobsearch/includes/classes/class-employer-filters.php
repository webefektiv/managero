    ...
    static function jobsearch_employer_filter_team_size_box_html_callback($html, $global_rand_id, $args_count, $left_filter_count_switch, $sh_atts) {
        ...
        <div class="jobsearch-filter-responsive-wrap">
            <div class="jobsearch-search-filter-wrap jobsearch-search-filter-toggle <?php echo ($team_filter_collapse == 'yes' ? 'jobsearch-remove-padding' : '') ?>">
                <h2><a href="javascript:void(0);" data-cname="<?php echo ($filter_collapse_cname) ?>" data-cval="<?php echo ($filter_collapse_cval) ?>" class="jobsearch-click-btn"><?php echo esc_html__('Team Size', 'wp-jobsearch'); ?></a></h2>
    ...
    static function jobsearch_employer_filter_location_box_html_callback($html, $global_rand_id, $args_count, $left_filter_count_switch, $sh_atts) {
        ...
        <div class="jobsearch-filter-responsive-wrap">
            <div class="jobsearch-search-filter-wrap jobsearch-search-filter-toggle <?php echo ($loc_filter_collapse == 'yes' ? 'jobsearch-remove-padding' : '') ?>">
                <h2><a href="javascript:void(0);" data-cname="<?php echo ($filter_collapse_cname) ?>" data-cval="<?php echo ($filter_collapse_cval) ?>" class="jobsearch-click-btn"><?php echo esc_html__('Locations', 'wp-jobsearch'); ?></a></h2>
                <?php
                        ...
                        if ($all_locations_type == 'api') {
                            ...
                            <ul class="jobsearch-row jobsearch-employer-profile-form">
                                            ...
                                            <select name="location_location1" <?php echo ('class="countries' . ($contries_class . $continents_class) . '" id="countryId"') ?>>
                                                ...
                                                if ($is_ajax) {
                                                    foreach ($api_contries_list as $api_cntry_key => $api_cntry_val) {
                                                        ?>
                                                        <option value="<?php echo ($api_cntry_val) ?>" <?php echo ($api_cntry_val == $loc_location1 ? 'selected="selected"' : '') ?> countryid="<?php echo ($api_cntry_key) ?>"><?php echo ($api_cntry_val) ?></option>
                                                        <?php
                                                    }
                                                }
                                                ?>
                                ...
                                <li class="jobsearch-column-12">
                                    ...
                                    if ($loc_optionstype == '2' || $loc_optionstype == '3') {
                                        ?>
                                        <input type="hidden" name="location_location1" value="<?php echo ($nameof_singl_contry) ?>"/>
                                        <input type="hidden" <?php echo ('id="countryId"') ?> value="<?php echo ($contry_singl_contry) ?>"/>
                                        <?php
                                    }
                                    ?>
                                                ...
                                                if ($states_cntry != '') {
                                                    ...
                                                    if ($selected_contry_key != '') {
                                                        ...
                                                        foreach ($api_states_list as $api_state_key => $api_state_val) {
                                                            ?>
                                                            <option value="<?php echo ($api_state_val) ?>" <?php echo ($api_state_val == $loc_location2 ? 'selected="selected"' : '') ?> stateid=<?php echo ($api_state_key) ?>><?php echo ($api_state_val) ?></option>
                                                            <?php
                                                        }
                                                    }
                                                }
                                                ...
                                                if ($is_ajax) {
                                                    if (isset($api_states_list) && !empty($api_states_list) && $loc_location2 != '') {
                                                        ...
                                                        foreach ($api_cities_list as $api_city_key => $api_city_val) {
                                                            ?>
                                                            <option value="<?php echo ($api_city_val) ?>" <?php echo ($api_city_val == $loc_location3 ? 'selected="selected"' : '') ?> cityid="<?php echo ($api_city_key) ?>"><?php echo ($api_city_val) ?></option>
                                                            <?php
                                                        }
                                                    }
                                                    //
                            ...
                            <div class="onsubmit-apilocs-con">
                                <a href="javascript:void(0);" class="jobsearch-onsubmit-apilocs btn jobsearch-bgcolor" onclick="jobsearch_employer_content_load(<?php echo absint($global_rand_id); ?>);"><?php esc_html_e('Submit', 'wp-jobsearch') ?></a>
                            </div>
                            <?php
                        } else {
