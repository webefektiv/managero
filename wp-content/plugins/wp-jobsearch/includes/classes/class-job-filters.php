...
class Jobsearch_JobFilterHTML {
	...
	public function __construct() {
		add_filter( 'jobsearch_job_filter_date_posted_box_html', array(
			$this,
			'jobsearch_job_filter_date_posted_box_html_callback'
		), 1, 5 );
		add_filter( 'jobsearch_job_filter_jobtype_box_html', array(
			$this,
			'jobsearch_job_filter_jobtype_box_html_callback'
		), 1, 5 );
		add_filter( 'jobsearch_job_filter_joblocation_box_html', array(
			$this,
			'jobsearch_job_filter_joblocation_box_html_callback'
		), 1, 5 );
		add_filter( 'jobsearch_job_filter_sector_box_html', array(
			$this,
			'jobsearch_job_filter_sector_box_html_callback'
		), 1, 5 );
	...
	static function jobsearch_job_filter_date_posted_box_html_callback( $html, $global_rand_id, $args_count, $left_filter_count_switch, $sh_atts ) {
        ...
        <div class="jobsearch-filter-responsive-wrap">
            <div class="jobsearch-search-filter-wrap jobsearch-search-filter-toggle <?php echo( $date_filter_collapse == 'yes' ? 'jobsearch-remove-padding' : '' ) ?>">
                <h2><a href="javascript:void(0);" data-cname="<?php echo( $filter_collapse_cname ) ?>"
                       data-cval="<?php echo( $filter_collapse_cval ) ?>"
                       class="jobsearch-click-btn"><?php echo esc_html__( 'Date Posted', 'wp-jobsearch' ); ?></a></h2>
                    ...
                    <ul class="jobsearch-checkbox">
                        <li<?php echo( $left_filter_count_switch != 'yes' ? ' class="no-filter-counts"' : '' ) ?>>
                            ...
                            <input id="lasthour<?php echo absint( $rand ); ?>" type="radio"
                                   name="posted" <?php if ( $posted == 'lasthour' ) {
                        ...
                        <li<?php echo( $left_filter_count_switch != 'yes' ? ' class="no-filter-counts"' : '' ) ?>>
                            ...
                            <input id="last24<?php echo absint( $rand ); ?>" type="radio"
                                   name="posted" <?php if ( $posted == 'last24' ) {
								echo 'checked="checked"';
							} ?> onchange="jobsearch_job_content_load(<?php echo absint( $global_rand_id ); ?>);"
                                   value="last24"/>
                        ...
                        <li<?php echo( $left_filter_count_switch != 'yes' ? ' class="no-filter-counts"' : '' ) ?>>
                            ...
                            <input id="7days<?php echo absint( $rand ); ?>" type="radio"
                                   name="posted" <?php if ( $posted == '7days' ) {
								echo 'checked="checked"';
							} ?> onchange="jobsearch_job_content_load(<?php echo absint( $global_rand_id ); ?>);"
                                   value="7days"/>
                        ...
                        <li<?php echo( $left_filter_count_switch != 'yes' ? ' class="no-filter-counts"' : '' ) ?>>
                            ...
                            <input id="14days<?php echo absint( $rand ); ?>" type="radio"
                                   name="posted" <?php if ( $posted == '14days' ) {
								echo 'checked="checked"';
							} ?> onchange="jobsearch_job_content_load(<?php echo absint( $global_rand_id ); ?>);"
                                   value="14days"/>
                        ...
                        <li<?php echo( $left_filter_count_switch != 'yes' ? ' class="no-filter-counts"' : '' ) ?>>
                            ...
                            <input id="30days<?php echo absint( $rand ); ?>" type="radio"
                                   name="posted" <?php if ( $posted == '30days' ) {
								echo 'checked="checked"';
							} ?> onchange="jobsearch_job_content_load(<?php echo absint( $global_rand_id ); ?>);"
                                   value="30days"/>
                        ...
                        <li<?php echo( $left_filter_count_switch != 'yes' ? ' class="no-filter-counts"' : '' ) ?>>
                            ...
                            <input id="all<?php echo absint( $rand ); ?>" type="radio"
                                   name="posted" <?php if ( $posted == 'all' || $posted == '' ) {
								echo 'checked="checked"';
							} ?> onchange="jobsearch_job_content_load(<?php echo absint( $global_rand_id ); ?>);"
                                   value="all"/>
	...
	static function jobsearch_job_filter_jobtype_box_html_callback( $html, $global_rand_id, $args_count, $left_filter_count_switch, $sh_atts ) {
        ...
        <div class="jobsearch-filter-responsive-wrap">
            <div class="jobsearch-search-filter-wrap jobsearch-search-filter-toggle <?php echo( $type_filter_collapse == 'yes' ? 'jobsearch-remove-padding' : '' ) ?>">
                <h2><a href="javascript:void(0);" data-cname="<?php echo( $filter_collapse_cname ) ?>"
                       data-cval="<?php echo( $filter_collapse_cval ) ?>"
                       class="jobsearch-click-btn"><?php echo esc_html__( 'Job Type', 'wp-jobsearch' ); ?></a></h2>
                <div class="jobsearch-checkbox-toggle"
	...
	static function jobsearch_job_filter_joblocation_box_html_callback( $html, $global_rand_id, $args_count, $left_filter_count_switch, $sh_atts  ) {
        ...
        <div class="jobsearch-filter-responsive-wrap">
            <div class="jobsearch-search-filter-wrap jobsearch-search-filter-toggle <?php echo( $loc_filter_collapse == 'yes' ? 'jobsearch-remove-padding' : '' ) ?>">
                <h2><a href="javascript:void(0);" data-cname="<?php echo( $filter_collapse_cname ) ?>"
                       data-cval="<?php echo( $filter_collapse_cval ) ?>"
                       class="jobsearch-click-btn"><?php echo esc_html__( 'Locations', 'wp-jobsearch' ); ?></a></h2>
				<?php
                    ...
                    <div class="jobsearch-checkbox-toggle"
						...
						if ( $all_locations_type == 'api' ) {
                            ...
                            <script>
								...
								if ($is_ajax) {
								?>
                                jQuery(document).on('change', 'select[name="location_location1"]', function () {
                                    var selctdContryId = jQuery(this).find(":selected").attr('countryid');

                                    jobsearch_custm_getJSON('http://geodata.solutions/api/api.php?type=getStates&countryId=' + selctdContryId + '&addClasses=order-alpha', function (err, data) {
                                        if (typeof data.result !== 'undefined') {
                                            ...
                                            jQuery.each(data.result, function (cnrty_indx, cntry_elm) {
                                                toStatesData += '<option value="' + cntry_elm + '" stateid=' + cnrty_indx + '>' + cntry_elm + '</option>';
                                            });
                                            jQuery('#jobsearch-gdapilocs-statecon').find('select').html(toStatesData);
                                        }
                                    });
                                });
                                jQuery(document).on('change', 'select[name="location_location2"]', function () {
                                    var selctdContryId = '';
									<?php
									...
									?>
                                    selctdContryId = jQuery('select[name="location_location1"]').find(":selected").attr('countryid');
									<?php
									}
									?>
                                    var selctdStateId = jQuery(this).find(":selected").attr('stateid');

                                    jobsearch_custm_getJSON('http://geodata.solutions/api/api.php?type=getCities&countryId=' + selctdContryId + '&stateId=' + selctdStateId + '&addClasses=order-alpha', function (err, data) {
                                        if (typeof data.result !== 'undefined') {
                                            var toCitiesData = '<option value=""><?php esc_html_e( 'Select City', 'wp-jobsearch' ) ?></option>';
                                            jQuery.each(data.result, function (cnrty_indx, cntry_elm) {
                                                toCitiesData += '<option value="' + cntry_elm + '" cityid=' + cnrty_indx + '>' + cntry_elm + '</option>';
                                            });
                                            jQuery('#jobsearch-gdapilocs-citycon').find('select').html(toCitiesData);
                                        }
                                    });
                                });
								<?php
                            ...
                            <ul class="jobsearch-row jobsearch-employer-profile-form">
								...
								if ( $loc_optionstype == '0' || $loc_optionstype == '1' ) {
                                    ...
                                    <li class="jobsearch-column-12">
                                        ...
                                        <div id="jobsearch-gdapilocs-contrycon"
                                             data-val="<?php echo( $loc_location1 ) ?>"
                                            ...
                                            <select name="location_location1" <?php echo( 'class="countries' . ( $contries_class . $continents_class ) . '" id="countryId"' ) ?>>
												...
												if ( $is_ajax ) {
													foreach ( $api_contries_list as $api_cntry_key => $api_cntry_val ) {
														?>
                                                        <option value="<?php echo( $api_cntry_val ) ?>" <?php echo( $api_cntry_val == $loc_location1 ? 'selected="selected"' : '' ) ?>
                                                                countryid="<?php echo( $api_cntry_key ) ?>"><?php echo( $api_cntry_val ) ?></option>
														<?php
													}
												}
												?>
                                ...
                                <li class="jobsearch-column-12">
									...
									if ( $loc_optionstype == '2' || $loc_optionstype == '3' ) {
                                        ...
                                        <input type="hidden" name="location_location1"
                                               value="<?php echo( $nameof_singl_contry ) ?>"/>
            ...
            <ul class="jobsearch-checkbox"> ';
							$job_location_flag = 1;
							echo self::get_terms_hierarchical( $all_job_type, '', 0, 0, $global_rand_id, $args_count,
								$left_filter_count_switch );
							echo '
            </ul>';
						} else {
						?>
							...
							if ( $loc_counter > 6 ) {
								...
								$agrs_count_json = str_replace( array( '<', '>' ), array(
									'lest_sign',
									'gret_sign'
								), $agrs_count_json );
	...
	static function jobsearch_job_filter_sector_box_html_callback( $html, $global_rand_id, $args_count, $left_filter_count_switch, $sh_atts ) {
        ...
        <div class="jobsearch-filter-responsive-wrap">
            <div class="jobsearch-search-filter-wrap jobsearch-search-filter-toggle <?php echo( $sec_filter_collapse == 'yes' ? 'jobsearch-remove-padding' : '' ) ?>">
                <h2><a href="javascript:void(0);" data-cname="<?php echo( $filter_collapse_cname ) ?>"
                       data-cval="<?php echo( $filter_collapse_cval ) ?>"
                       class="jobsearch-click-btn"><?php echo esc_html__( 'Sector', 'wp-jobsearch' ) ?></a></h2>
                <div class="jobsearch-checkbox-toggle"
					...
					if ( $sector != '' ) {
                        ...
                        <ul class="jobsearch-checkbox">
                            <li><a href="#"
                                   onclick="jobsearch_job_content_load(<?php echo absint( $global_rand_id ); ?>);"><?php echo esc_html__( 'View all sector', 'wp-jobsearch' ); ?></a>
                            </li>
                            <li>&nbsp;</li>
                        </ul>
						<?php
						...
						if ( $show_sector == 'yes' ) {
							...
							foreach ( $all_sector as $sectoritem ) {
								...
								if ( $input_type_sector == 'checkbox' ) {
									...
									<?php
								} else if ( $input_type_sector == 'radio' ) {
									$sector_selected = '';
									if ( $sector == $sectoritem->slug ) {
										$sector_selected = ' checked="checked"';
									}
