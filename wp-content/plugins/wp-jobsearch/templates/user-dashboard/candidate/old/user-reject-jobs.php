...
if ( $candidate_id > 0 ) {
	...
	if ( ! empty( $candidate_rej_jobs_list ) ) {
		...
		$candidate_rej_jobs_list = '';
	}
	?>
    <div class="jobsearch-employer-box-section">
        <div class="jobsearch-profile-title">
            <h2><?php echo apply_filters( 'jobsearch_cand_dash_favjobs_mainhead_title', esc_html__( 'Joburi neinteresante', 'wp-jobsearch' ) ) ?></h2>
        </div>
		<?php
		if ( $candidate_rej_jobs_list != '' ) {
			...
			if ( ! empty( $candidate_rej_jobs_list ) ) {
				$total_jobs = count( $candidate_rej_jobs_list );
				krsort( $candidate_rej_jobs_list );

				$start  = ( $page_num - 1 ) * ( $reults_per_page );
				$offset = $reults_per_page;

				$candidate_rej_jobs_list = array_slice( $candidate_rej_jobs_list, $start, $offset );
                    ...
                    <ul class="jobsearch-row">
                        ...
                        <div class="jobsearch-column-12">
                            <div class="jobsearch-table-layer jobsearch-managejobs-tbody manage3">
								...
								<?php
								foreach ( $candidate_rej_jobs_list as $job_id ) {
                                    ...
                                    <div class="jobsearch-table-row" data-job="<?= $job_id; ?>">
                                        ...
                                        <div class="jobsearch-table-cell">
                                            <a href="javascript:void(0);"
                                               class="jobsearch-savedjobs-links jobsearch-delete-rej-job"
				...
				echo apply_filters( 'jobsearch_cand_dash_favjobs_list_html', $favjobs_html, $candidate_rej_jobs_list, $candidate_id );
