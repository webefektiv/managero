...
// hook things up
    ...
    static function jobsearch_custom_field_video_load_callback($html, $post_id, $custom_field_saved_data, $fields_prefix) {
        ...
        <div class="jobsearch-element-field">
            ...
            <div class="elem-field">
                <input type="text" name="<?php echo esc_html($video_field_name) ?>" class="<?php echo esc_html($video_field_classes) ?>" placeholder="<?php echo esc_html($video_field_placeholder) ?>" <?php echo force_balance_tags($video_field_required_str) ?> value="<?php echo esc_html($video_field_name_db_val) ?>" />
            </div>
        </div>
        <?php
        $html .= ob_get_clean();
    ...
    static function jobsearch_custom_field_linkurl_load_callback($html, $post_id, $custom_field_saved_data, $fields_prefix) {
        ...
        <div class="jobsearch-element-field">
            ...
            <div class="elem-field">
                <input type="text" name="<?php echo esc_html($linkurl_field_name) ?>" class="<?php echo esc_html($linkurl_field_classes) ?>" placeholder="<?php echo esc_html($linkurl_field_placeholder) ?>" <?php echo force_balance_tags($linkurl_field_required_str) ?> value="<?php echo esc_html($linkurl_field_name_db_val) ?>" />
            </div>
        </div>
        <?php
        $html .= ob_get_clean();
        ...
        <div class="jobsearch-element-field">
            ...
            <div class="elem-field">
                <div class="jobsearch-fileUpload">
                    <input id="att-upload-files-<?php echo ($rand_num) ?>" type="button" class="upload jobsearch-upload" value="<?php esc_html_e('Upload Files', 'wp-jobsearch') ?>"/>
                </div>
                            ...
                            foreach ($all_attach_files as $_attach_file) {
                                ...
                                if ($_attach_id > 0) {
                                    ...
                                    <li class="jobsearch-column-3">
                                        ...
                                        <input type="hidden" name="<?php echo ($post_files_name) ?>[]" value="<?php echo ($_attach_file) ?>">
                                    </li>
                                    <?php
                                }
                            }
    ...
    static function jobsearch_dashboard_custom_fields_load_callback($post_id, $custom_field_entity) {
        ...
        if (is_array($custom_all_fields_saved_data) && sizeof($custom_all_fields_saved_data) > 0) {
            ...
            <div class="jobsearch-profile-title" style="display:none;"><h2>' . esc_html__('Other Information', 'wp-jobsearch') . '</h2></div>
            <ul class="jobsearch-row jobsearch-employer-profile-form">';

//            print_r($custom_all_fields_saved_data);
//            die();
    ...
    static function jobsearch_dashboard_custom_field_video_load_callback($html, $post_id, $custom_field_saved_data, $fields_prefix, $ffield_attr = array()) {
        ...
        <li class="jobsearch-column-6">
            ...
            <input type="text" name="<?php echo esc_html($video_field_name) ?>" class="<?php echo esc_html($video_field_classes) ?>" placeholder="<?php echo apply_filters('wpml_translate_single_string', $video_field_placeholder, 'Custom Fields', 'Text Field Placeholder - ' . $video_field_placeholder, $lang_code) ?>" <?php echo force_balance_tags($video_field_required_str) ?> value="<?php echo esc_html($video_field_name_db_val) ?>" />
        </li>
        <?php
        $html .= ob_get_clean();
    ...
    static function jobsearch_dashboard_custom_field_linkurl_load_callback($html, $post_id, $custom_field_saved_data, $fields_prefix, $ffield_attr = array()) {
        ...
        <li class="jobsearch-column-6">
            ...
            <input type="text" name="<?php echo esc_html($linkurl_field_name) ?>" class="<?php echo esc_html($linkurl_field_classes) ?>" placeholder="<?php echo apply_filters('wpml_translate_single_string', $linkurl_field_placeholder, 'Custom Fields', 'Text Field Placeholder - ' . $linkurl_field_placeholder, $lang_code) ?>" <?php echo force_balance_tags($linkurl_field_required_str) ?> value="<?php echo esc_html($linkurl_field_name_db_val) ?>" />
        </li>
        <?php
        $html .= ob_get_clean();
    ...
    static function jobsearch_dashboard_custom_field_upload_file_load_callback($html, $post_id, $custom_field_saved_data, $fields_prefix) {
        ...
        <li class="jobsearch-column-12">
            ...
            <div class="jobsearch-fileUpload">
                ...
                <input name="<?php echo esc_html($upload_file_field_name) ?>[]" type="file" class="upload jobsearch-upload jobsearch-uploadfile-field" multiple="multiple" onchange="jobsearch_job_attach_files_url_<?php echo ($rand_num) ?>(event)" />
                <?php
                if ($upload_field_numof_files > 1) {
                    ?>
                    <div class="jobsearch-fileUpload-info">
                        <p><?php printf(esc_html__('You can upload upto %s files.', 'wp-jobsearch'), $upload_field_numof_files); ?></p>
                    </div>
                    <?php
                }
                ?>
                    ...
                    if (window.File && window.FileList && window.FileReader) {
                        ...
                        for (var i = 0; i < files.length; i++) {
                            ...
                            if (file_size <= file_allow_size) {
                                ...
                                alert('<?php esc_html_e('File size is too large.') ?>');
                                return false;
                            }
    ...
    static function jobsearch_dashboard_custom_field_date_load_callback($html, $post_id, $custom_field_saved_data, $fields_prefix, $ffield_attr = array()) {
        ...
        $date_field_name_db_val = get_post_meta($post_id, $date_field_name, true);
        if ($date_field_name_db_val != '') {
            $date_field_name_db_val = date($date_field_date_format, strtotime($date_field_name_db_val));
        }
        ...
        <li class="jobsearch-column-6">
            ...
            if ($range_field_style == 'slider') {
                ...
                <div class="jobsearch-profile-select">
                    <select name="<?php echo esc_html($range_field_name) ?>" class="<?php echo ($range_field_classes) ?> selectize-select" <?php echo ($range_field_placeholder != '' ? 'placeholder="' . $range_field_placeholder . '"' : '') ?> <?php echo ($range_field_required == 'yes' ? 'required="required"' : '') ?>>
                        <?php
    ...
    static function jobsearch_dashboard_custom_field_dropdown_load_callback($html, $post_id, $custom_field_saved_data, $fields_prefix, $ffield_attr = array()) {
        ...
        <li class="jobsearch-column-6">
            ...
            if ($dropdown_field_options_str != '') {
                ...
                ?>
                <div class="jobsearch-profile-select">
                    <select <?php echo ($field_post_multi == 'yes' ? 'multiple="multiple" ' : '') ?>name="<?php echo esc_html($dropdown_field_name) ?><?php echo ($field_post_multi == 'yes' ? '[]' : '') ?>" placeholder="<?php echo apply_filters('wpml_translate_single_string', $dropdown_field_placeholder, 'Custom Fields', 'Dropdown Field Placeholder - ' . $dropdown_field_placeholder, $lang_code) ?>" class="<?php echo esc_html($dropdown_field_classes) ?> selectize-select" <?php echo force_balance_tags($dropdown_field_required_str) ?>>
                        <?php
    ...
    static function jobsearch_dashboard_custom_field_dependent_dropdown_load_callback($html, $post_id, $custom_field_saved_data, $fields_prefix, $ffield_attr = array()) {
        ...
        ?>
        <li class="jobsearch-column-6">
            <label><?php echo ($dropdown_field_label) ?></label>
            <div class="jobsearch-depdrpdwn-fields <?php echo ($dropdown_field_classes) ?>">
                <?php
                $depdrpdwn_fields = jobsearch_dependent_dropdown_list_html($dropdown_field_options, $dropdown_cont_optsid, $custom_field_saved_data, $dropdown_field_name_db_val);
                echo ($depdrpdwn_fields);
    ...
    static function jobsearch_form_custom_field_dropdown_load_callback($html, $post_id, $custom_field_saved_data, $fields_prefix, $con_class = '', $f_display = '') {
        ...
        <li<?php echo ($con_clas_attr) ?><?php echo ($con_f_style) ?>>
            ...
            if ($dropdown_field_options_str != '') {
                ?>
                <div class="jobsearch-profile-select">
                    <select <?php echo ($field_post_multi == 'yes' ? 'multiple="multiple" ' : '') ?>name="<?php echo esc_html($dropdown_field_name) ?><?php echo ($field_post_multi == 'yes' ? '[]' : '') ?>" <?php echo ($dropdown_field_placeholder != '' ? 'placeholder="' . $dropdown_field_placeholder . '"' : '') ?> class="<?php echo esc_html($dropdown_field_classes) ?> selectize-select <?php echo ($dropdown_field_required_str) ?>">
                        <?php
                        echo force_balance_tags($dropdown_field_options_str);
                        ?>
                    </select>
    ...
    static function jobsearch_form_custom_field_dependent_dropdown_load_callback($html, $post_id, $custom_field_saved_data, $fields_prefix, $con_class = '', $f_display = '') {
        ...
        ?>
        <li<?php echo ($con_clas_attr) ?><?php echo ($con_f_style) ?>>
            <label><?php echo ($dropdown_field_label) ?> <?php echo ($dropdown_field_required == 'yes' ? '*' : '') ?></label>
            <div class="jobsearch-depdrpdwn-fields <?php echo ($dropdown_field_classes) ?>">
                <?php
                $depdrpdwn_fields = jobsearch_dependent_dropdown_list_html($dropdown_field_options, $dropdown_cont_optsid, $custom_field_saved_data, $dropdown_field_name_db_val);
                echo ($depdrpdwn_fields);
                ...
                foreach ($jobsearch_post_type_cus_fields as $cus_fieldvar => $cus_field) {
                    ...
                    if (isset($cus_field['name']) && $cus_field['name'] <> '') {
                        ...
                        if ($type == 'dependent_dropdown') {
                            $dep_drpdwn_valle = explode('|', $cus_field_value_arr);
                            $cus_field_value_arr = implode(', ', $dep_drpdwn_valle);
                        }
                        //
                        ...
                        if (($type == 'textarea' || $type == 'video') && strpos($before_html, $prefix . '-column-4') !== false) {
            ...
            foreach ($job_cus_fields as $cus_fieldvar => $cus_field) {
                ...
                if (isset($cus_field['enable-search']) && $cus_field['enable-search'] == 'yes' && ($all_item_empty == 0)) {
                    ...
                    if ($cus_field['type'] == 'heading' && $cus_field_label_arr != '') {
                        ...
                        $filter_collapse_cname = sanitize_title($cus_field['name']) . '_csec_collapse';
                        ...
                        <div class="jobsearch-filter-responsive-wrap">
                            <div class="jobsearch-search-filter-wrap <?php echo ($collapse_condition == 'yes' ? 'jobsearch-search-filter-toggle jobsearch-remove-padding' : 'jobsearch-search-filter-toggle') ?>">
                                <h2>
                                    <a href="javascript:void(0);" data-cname="<?php echo ($filter_collapse_cname) ?>" data-cval="<?php echo ($filter_collapse_cval) ?>" class="jobsearch-click-btn">
...
//                                                 
                                        ...
                                        <div class="jobsearch-depdrpdwn-fields">
                                            ...
                                            echo ($depdrpdwn_fields);
                                            ?>
                                            <a href="javascript:void(0);" class="depdrpdwn-form-submitbtn jobsearch-bgcolor btn" onclick="<?php echo ($submit_js_function_str); ?>"><?php esc_html_e('Submit', 'wp-jobsearch') ?></a>
                                        </div>
                                        <?php
                                            ...
                                            <li>
                                                <div class="filter-datewise-con">
                                                    ...
                                                    <label for="from<?php echo esc_html($query_str_var_name) . $global_rand_id; ?>">
                                                        <input type="text" name="from-<?php echo esc_html($query_str_var_name); ?>" id="from<?php echo esc_html($query_str_var_name) . $global_rand_id; ?>" value="<?php echo esc_html($fromdate_field_req_val); ?>" placeholder="<?php esc_html_e('Date From', 'wp-jobsearch') ?>" onchange="<?php echo force_balance_tags($submit_js_function_str); ?>" />
                                                    </label>
                                                    <label for="to<?php echo esc_html($query_str_var_name) . $global_rand_id; ?>">
                                                        <input type="text" name="to-<?php echo esc_html($query_str_var_name); ?>" id="to<?php echo esc_html($query_str_var_name) . $global_rand_id; ?>" value="<?php echo esc_html($todate_field_req_val); ?>" placeholder="<?php esc_html_e('Date To', 'wp-jobsearch') ?>" onchange="<?php echo force_balance_tags($submit_js_function_str); ?>" />
                                                    </label>
                                                </div>
                                            </li>
                                        </ul>
    ...
    static function custom_fields_top_filter_box_html_callback($html, $custom_field_entity = '', $global_rand_id) {
        ...
        if (!empty($job_cus_fields)) {
            foreach ($job_cus_fields as $cus_fieldvar => $cus_field) {
                ...
                if (isset($cus_field['enable-advsrch']) && $cus_field['enable-advsrch'] == 'yes' && ($all_item_empty == 0)) {
                    if ($cus_field['type'] == 'salary') {
                        $query_str_var_name = 'jobsearch_field_job_salary';
                        $str_salary_type_name = 'job_salary_type';
                        if ($custom_field_entity == 'candidate') {
                    ...
                    if ($cus_field['type'] == 'dropdown') {
                        ...
                        ?>
                        <li>
                            <input type="text" name="<?php echo esc_html($query_str_var_name); ?>" id="<?php echo esc_html($query_str_var_name); ?>" placeholder="<?php echo ($cus_field_label_arr) ?>" value="<?php echo esc_html($text_field_req_val); ?>" />
                        </li>
                        <?php
                        ...
                        ?>
                        <li>
                            <textarea name="<?php echo esc_html($query_str_var_name); ?>" id="<?php echo esc_html($query_str_var_name); ?>" placeholder="<?php echo ($cus_field_label_arr) ?>"><?php echo esc_html($text_field_req_val); ?></textarea>
                        </li>
                        <?php
                    } else if ($cus_field['type'] == 'number') {
                        $number_field_req_val = isset($_REQUEST[$query_str_var_name]) ? $_REQUEST[$query_str_var_name] : '';
                        ?>
                        <li>
                            <input type="number" name="<?php echo esc_html($query_str_var_name); ?>" id="<?php echo esc_html($query_str_var_name); ?>" placeholder="<?php echo ($cus_field_label_arr) ?>" value="<?php echo esc_html($number_field_req_val); ?>" />
                        </li>
                        <?php
                    } else if ($cus_field['type'] == 'date') {
                        $fromdate_field_req_val = isset($_REQUEST['from-' . $query_str_var_name]) ? $_REQUEST['from-' . $query_str_var_name] : '';
                        ...
                        <li>
                            <div class="filter-datewise-con">
                                ...
                                <label for="from<?php echo esc_html($query_str_var_name); ?>">
                                    <input type="text" name="from-<?php echo esc_html($query_str_var_name); ?>" id="from<?php echo esc_html($query_str_var_name); ?>" placeholder="<?php esc_html_e('Date From', 'wp-jobsearch') ?>" value="<?php echo esc_html($fromdate_field_req_val); ?>" />
                                </label>
                                <label for="to<?php echo esc_html($query_str_var_name); ?>">
                                    <input type="text" name="to-<?php echo esc_html($query_str_var_name); ?>" id="to<?php echo esc_html($query_str_var_name); ?>" placeholder="<?php esc_html_e('To From', 'wp-jobsearch') ?>" value="<?php echo esc_html($todate_field_req_val); ?>" />
                                </label>
                            </div>
                        </li>
                        <?php
