...
// hook things up
    ...
    public function jobsearch_custom_fields_save_callback() {
        ...
        $heading_counter = $textarea_counter = $dropdown_counter = $date_counter = $email_counter = $upload_counter = $text_counter = $linkurl_counter = $video_counter = $number_counter = $dependent_dropdown_counter = $range_counter = $salary_counter = $error = 0;
            ...
            foreach ($post_data['jobsearch-custom-fields-type'] as $custom_fields_type) {
                ...
                switch ($custom_fields_type) {
                    ...
                    case('dependent_dropdown'):
                        ...
                        if (isset($_POST["jobsearch-custom-fields-{$custom_fields_type}"]['options'][0]['id'][$dropdown_cont_optsid]) && !empty($_POST["jobsearch-custom-fields-{$custom_fields_type}"]['options'][0]['id'][$dropdown_cont_optsid])) {
                            $custom_field_array['options_list'] = array();
                            $custom_field_array['options_list'] = $_POST["jobsearch-custom-fields-{$custom_fields_type}"]['options'];
                        } else {
                            $error = 1;
                            $error_msg .= esc_html__('Please select atleast one option for field.', 'wp-jobsearch') . "'<br/>";
                        }
