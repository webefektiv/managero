...
// <a href="#" class="widget_jobdetail_three_apply_btn"><i class="careerfy-icon careerfy-heart"></i> Save this Job</a>
        ...
        public function jobsearch_job_shortlist_button_callback($args = '') {
            ...
            if ($view == 'job_detail_3') {
                if (!is_user_logged_in()) {
                    ?>
                    <a href="javascript:void(0);" class="<?php echo ($anchor_class); ?> jobsearch-open-signin-tab"><i class="<?php echo $before_icon; ?>"></i> <?php echo ($before_label); ?></a>
                    <?php
                } else {
                    ...
                    ?>
                    <a href="javascript:void(0);" data-after-label="<?php echo ($after_label); ?>" data-view="job3" class="<?php echo in_array($job_id, $candidate_fav_jobs_list) ? '' : 'jobsearch-add-job-to-favourite' ?> <?php echo ($anchor_class); ?>" data-id="<?php echo ($job_id); ?>" data-before-icon='<?php echo ($before_icon); ?>' data-after-icon='<?php echo ($after_icon); ?>'>
                ...
                if (!is_user_logged_in()) {
                    ?>
                    <a href="javascript:void(0);" class="shortlist_job_btn jobsearch-open-signin-tab"><?php echo ($before_label); ?></a>
                    <?php
                } else {
                    $user_id = get_current_user_id();
                    $user_is_candidate = jobsearch_user_is_candidate($user_id);
                    ...
                    ?>
                    <a href="javascript:void(0);" data-after-label="<?php echo ($after_label); ?>" data-view="job2" class="shortlist_job_btn <?php echo in_array($job_id, $candidate_fav_jobs_list) ? '' : 'jobsearch-add-job-to-favourite' ?>" data-id="<?php echo ($job_id); ?>" data-before-icon="<?php echo ($before_icon); ?>" data-after-icon="<?php echo ($after_icon); ?>">
                    <?php echo '<i class=""></i>';
                ...
                if (!is_user_logged_in()) {
                    ...
                    ?>
                    <div class="like-btn <?php echo ($container_class) ?>">
                        <a href="javascript:void(0);"  data-toggle="tooltip" title="<?php echo in_array($job_id, $candidate_fav_jobs_list) ? 'Sterge din lista de rezervate' : 'Aplica mai tarziu' ?>"  class="shortlist <?php echo in_array($job_id, $candidate_fav_jobs_list) ? 'jobsearch-delete-fav-job' : 'jobsearch-add-job-to-favourite' ?> <?php echo ($anchor_class) ?>" data-id="<?php echo ($job_id) ?>" data-before-icon="<?php echo ($before_icon) ?>" data-after-icon="<?php echo ($after_icon) ?>">
                            <i class="<?php echo in_array($job_id, $candidate_fav_jobs_list) ? $before_icon : $after_icon ?>"></i>
	                        <?php echo in_array($job_id, $candidate_fav_jobs_list) ? ($after_label) : ($before_label); ?>
                        </a>
                        <span class="job-to-fav-msg-con"></span>
        ...
        public function jobsearch_shortlist_submit_callback() {
            ...
            if ($user_is_candidate) {
                ...
                echo json_encode(array('msg' => esc_html__('Job added to list.', 'wp-jobsearch'),'state' => $state ,'job' => $job_id));
                die;
            } else {
